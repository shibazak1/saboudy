...
class PickOrder(BaseOrderAction):
    ...
    def post(self,request):
        ...
        print(f" order status 1: {order.status}")
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if order.status=="PE":
                    ...
                    print(f" order status 2 is {order.status}")
                    ...
                    url = reverse("all")
                    users = []
                    users.append(order.owner)
                    sendNotification(users,message,url)

                if distance<0.1:
                    self.change_status(order,"DE")
                    print(f" order status 3 is {order.status}")
